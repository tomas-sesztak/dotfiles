...
- Packages:
	...
	- `zsh/` → `~/.zshrc` + `~/.config/zsh/functions` (whole dir stowed). `~/.zshrc` globs and sources every `~/.config/zsh/functions/**/*.zsh` file. `~/.config/zsh/completions` is excluded — generated at runtime by `generate_completions()` (`functions/completions.zsh`), not tracked. `tmux-init-agents -a <agent> -p <path>` (in tmux) / `herdr-init-agents -a <agent> -p <path>` (herdr; auto-starts the herdr server if needed) each ensure a window/workspace per top-level git repo under `<path>`, launching `<agent>` in any missing ones. `herdr-fork-agent -a <agent> -w <name> [-c "<cmd>"]` (must run inside herdr; resolves the repo via `$HERDR_WORKSPACE_ID`) forks the current repo into a new herdr worktree and starts `<agent>` there, optionally sending `<cmd>` via `herdr agent prompt`. All three take `-h`/`--help`.
