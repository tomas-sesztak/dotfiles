...
- Stow packages:
	...
	- `bash/` — stows individual bash dotfiles (`.bashrc`, `.bash_profile`, `.bash_logout`) directly into `$HOME`, since (unlike `nvim`) there's no shared subdirectory to stow as a whole. `.bash_history` is intentionally excluded (runtime state, not config).
