...
- Packages:
	...
	- `zsh/` → `~/.zshrc` + `~/.config/zsh/functions` (whole dir stowed). `~/.zshrc` globs and sources every `~/.config/zsh/functions/**/*.zsh` file. `~/.config/zsh/completions` is excluded — generated at runtime by `generate_completions()` (`functions/completions.zsh`), not tracked. `init-agents -a <agent> -p <path>` ensures a window/workspace per top-level git repo under `<path>`, launching `<agent>` in any missing ones. It auto-detects the active multiplexer (herdr checked first via `$HERDR_ENV`/`$HERDR_WORKSPACE_ID`, then tmux via `$TMUX`; herdr mode auto-starts the herdr server if needed) and errors if run outside both. `herdr-fork-agent -a <agent> -w <name> [-c "<cmd>"]` (must run inside herdr; resolves the repo via `$HERDR_WORKSPACE_ID`) forks the current repo into a new herdr worktree and starts `<agent>` there, optionally sending `<cmd>` via `herdr agent prompt`. Both take `-h`/`--help`.
