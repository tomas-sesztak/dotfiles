...
- Packages:
	...
	- `zsh/` → `~/.zshrc` + `~/.config/zsh/functions` (whole dir stowed). `~/.zshrc` globs and sources every `~/.config/zsh/functions/**/*.zsh` file. `~/.config/zsh/completions` is excluded — it's generated at runtime by `generate_completions()` (defined in `functions/completions.zsh`), not tracked. `functions/tmux-init-agents.zsh` adds `tmux-init-agents -a <agent> -p <path>` (run from inside tmux; `-h`/`--help` for usage): for each top-level git repo under `<path>`, ensures a same-named tmux window exists, creating it with `<agent>` launched inside (cwd set to the repo) if missing. `functions/herdr-fork-agent.zsh` adds `herdr-fork-agent -a <agent> -w <worktree_name> [-c "<command>"]` (run from inside an active herdr pane; `-h`/`--help` for usage): forks the current project into a new herdr-managed git worktree (via `herdr worktree create --workspace "$HERDR_WORKSPACE_ID"`, which resolves the repo from the calling pane's workspace) and starts `<agent>` in it, optionally seeding it with `-c`'s text via `herdr agent prompt` once running.
